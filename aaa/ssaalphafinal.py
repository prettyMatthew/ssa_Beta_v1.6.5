...
# =========================
def get_itrs_xyz(obj):
    """
    Return ITRS xyz 'Distance-like' object for both:
      - callable method: itrs_xyz()
      - attribute: itrs_xyz
    """
    itrs = getattr(obj, "itrs_xyz", None)
    if itrs is None:
        raise AttributeError("Object has no itrs_xyz. Check Skyfield object type.")
    return itrs() if callable(itrs) else itrs

def to_xyz_km(obj) -> Tuple[float, float, float]:
    """
    Robust conversion to (x,y,z) km across Skyfield versions.
    Accepts:
      - Distance objects that store xyz as .km (tuple-like)
      - Vector/Position objects that store xyz as .position.km
      - plain iterables (x,y,z)
    """
    # Case A: has .km (often Distance with vector components)
    if hasattr(obj, "km"):
        km = obj.km
        # expect vector-like (x,y,z)
        x, y, z = km
        return float(x), float(y), float(z)

    # Case B: has .position.km (vector-like)
...
def build_trails_xyz(sats, ts_obj, dt_list, limit):
    ...
    for sat in sats[:limit]:
        ...
        for tt in t_list:
            sat_at = sat.at(tt)
            xyz = to_xyz_km(get_itrs_xyz(sat_at))
            pts.append(xyz)
...
# Ground XYZ (robust)
ground_xyz = to_xyz_km(get_itrs_xyz(obs))
...
for g, sats in groups.items():
    ...
    for elev, sat, az, dist_km, vis in elev_rank:
        ...
        sat_at = sat.at(t)
        x, y, z = to_xyz_km(get_itrs_xyz(sat_at))
        pts.append((x, y, z))
        hovers.append(
            f"{g} | {sat.name}"
